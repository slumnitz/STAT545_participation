...
unclass(my_lm) # truly shows list
names(my_lm)
my_lm$coefficients
```
...
*tidy*: Use to interpret how variables relate to each other. Model parameters are slope and intercep. And p-value for hypothesis testing.

*augment*: new predicted values

*glance*: the closer R-square to one the more x influences y. P-value significance to predict y wihtout using x, the smaller the more significant.